...
export default function ContactLogPage() {
  ...
  const loadByCustomer = async (customerId: string) => {
    ...
    setExpandedId(null)
    try {
      const data = await api.buyers.list(customerId)
      setBuyers(data)
      setFiltered(data)
    } finally {
      setLoading(false)
  ...
  useEffect(() => {
    const customer = customers.find(c => c.name === customerFilter)
  ...
  const saveLog = async (buyer: any, attemptNo: number) => {
    ...
    try {
      ...
      const customer = customers.find(c => c.name === customerFilter)
      if (customer) {
        const data = await api.buyers.list(customer.id)
        setBuyers(data)
        setFiltered(data)
      }
  ...
  const exportExcel = async () => {
    const customer = customers.find(c => c.name === customerFilter)
    if (!customer) { toast.error("Please select a client first"); return }
    const res = await api.buyers.exportExcel(customer.id)
      ...
      <div className="p-8 max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">
              {customerFilter
                ? `${filtered.length} buyers · ${customerFilter}`
                : "Select a client to view contact logs"}
            </p>
          </div>
          {customerFilter && (
        ...
        <div className="flex gap-3 mb-5">
          <select
            ...
            onChange={e => { setCustomerFilter(e.target.value); setSearch("") }}
            className={cn(
              "text-sm border rounded-xl px-3 py-2.5 bg-white focus:outline-none focus:ring-2 focus:ring-indigo-300 font-medium",
              customerFilter ? "border-indigo-300 text-indigo-700" : "border-slate-200 text-slate-500"
            )}>
        ...
        {!customerFilter && (
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm py-24 text-center">
            ...
            <p className="text-base font-semibold text-slate-600 mb-1">Select a client to view contact logs</p>
        ...
        {customerFilter && !loading && (
          <div className="space-y-2">
            {filtered.map(buyer => {
              ...
              const displayContacts = contacts.length > 0 ? contacts : [{ name: null, email: null, idx: 1 }]
              ...
              return (
                <div key={buyer.id} className={cn("bg-white rounded-2xl border shadow-sm overflow-hidden",
                  ...
                  {displayContacts.map((contact, ci) => (
                    <div key={`${buyer.id}-${contact.idx}`}
                      className={cn("flex items-center gap-4 px-5 py-3.5", ci > 0 && "border-t border-slate-50")}>
                      <button onClick={() => setExpandedId(isOpen ? null : buyer.id)}
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            {ci === 0 && <span className="text-xs text-slate-400">({buyer.country})</span>}
                            {displayContacts.length > 1 && (
                              <span className="text-[10px] bg-indigo-50 text-indigo-500 px-1.5 py-0.5 rounded-full font-medium shrink-0">
                                Contact {contact.idx}
                              </span>
                            )}
                          </div>
                    ...
                    <div className="border-t border-slate-100 px-5 py-4 bg-slate-50">
                      {isStopped && (
                        <div className="mb-3 flex items-center gap-2 bg-red-50 border border-red-200 rounded-xl px-4 py-2.5">
                          ...
                          <p className="text-xs text-red-600 font-medium">This buyer has requested to stop contact.</p>
                        </div>
                      )}
      ...
      {stopTarget && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl">
            ...
            <p className="text-sm text-slate-600 mb-1">Are you sure you want to stop contacting:</p>
